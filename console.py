...
class HBNBCommand(cmd.Cmd):
    ...
    def do_create(self, line):
        ...
        if not line:
            ...
            if line_list[0] not in my_classes:
                print("** class doesn't exist **")
                return
            for arg in line_list[1:]:
                arg_splited = arg.split("=")
                arg_splited[1] = eval(arg_splited[1])
                if type(arg_splited[1]) is str:
                    arg_splited[1] = arg_splited[1].replace(
                        "_", " ").replace('"', '\\"')
                parameters[arg_splited[0]] = arg_splited[1]
            myobj = my_classes[line_list[0]](**parameters)
            myobj.save()
            print(myobj.id)
    ...
    def do_all(self, args):
        ...
        if args:
            ...
            if args not in my_classes:
                print("** class doesn't exist **")
                return
            for k, v in storage.all(my_classes[args]).items():
